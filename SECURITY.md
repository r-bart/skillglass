...
Skill Forge has not published a supported public release yet. This section will identify supported release lines when the first release is published.
...
The only official binary-distribution channel is this repository's [Releases page](../../releases). Skill Forge does not publish binaries to app stores, mirrors, package registries, standalone download sites, or automatic-update endpoints.
...
- Skill Forge reads skills and writes only installations or updates inside explicitly approved, user-writable roots.
- Skill Forge does not request administrator, root, UAC, or other operating-system elevation and does not recommend changing permissions to make a write succeed.
- Skill Forge does not activate, deactivate, configure, or uninstall skills in a harness.
